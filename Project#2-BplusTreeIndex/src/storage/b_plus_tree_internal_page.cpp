...
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::ValueIndex(const ValueType& value) const {
    ...
    auto target = std::find_if(array, array + GetSize(),
                               [&value](const auto& pair) { return pair.second == value; });
    return std::distance(array, target);
}
...
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::InsertNodeAfter(const ValueType& old_value,
    ...
    std::move_backward(array + new_value_index, array + GetSize(), array + GetSize() + 1);

    array[new_value_index].first = new_key;
    array[new_value_index].second = new_value;
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveHalfTo(BPlusTreeInternalPage* recipient,
    ...
    recipient->CopyNFrom(array + start_index, GetMaxSize() - start_index, buffer_pool_manager);
}

/* Copy entries into me, starting from {items} and copy {size} entries.
 * Since it is an internal page, for all entries (pages) moved, their parents page now changes to
 * me. So I need to 'adopt' them by changing their parent page id, which needs to be persisted with
 * BufferPoolManger
 */
INDEX_TEMPLATE_ARGUMENTS
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::Remove(int index) {
    // move elements [ array+index+1,array+GetSize() ) to place start at array+index
 ...
 * So I need to 'adopt' it by changing its parent page id, which needs to be persisted with
 * BufferPoolManger
 */
INDEX_TEMPLATE_ARGUMENTS
 ...
 * You need to handle the original dummy key properly, e.g. updating recipientâ€™s array to position
 * the middle_key at the right place. You also need to use BufferPoolManager to persist changes to
 * the parent page id for those pages that are moved to the recipient
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveLastToFrontOf(BPlusTreeInternalPage* recipient,
                                                       const KeyType& middle_key,
                                                       BufferPoolManager* buffer_pool_manager) {
    MappingType last_item = array[GetSize() - 1];
