...
class MockBufferPoolManager : public BufferPoolManager {
   public:
    ...
    using bufferpool_callback_fn = void (MockBufferPoolManager::*)(enum CallbackType type,
                                                                   FuncType func_type);

    MockBufferPoolManager(size_t pool_size,
   ...
   private:
     ...
     * @return false if the page exists but could not be deleted, true if the page didn't exist or
     * deletion succeeded
     */
    ...
    /** This latch protects shared data structures. We recommend updating this comment to describe
     * what it protects. */
    std::mutex latch_;
};
